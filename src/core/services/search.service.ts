...
export class SearchService {
    ...
    searchMovies(key: string, page: number): Observable<GenericResponse<Movie[]>> {
        const url = `${this.basePath}/search/movie?query=${key}&include_adult=false&language=tr-tr&page=${page}`;
        return this.http.get<GenericResponse<Movie[]>>(url)
    }

    searchTvShows(key: string, page: number): Observable<GenericResponse<TvShow[]>> {
        const url = `${this.basePath}/search/tv?query=${key}&include_adult=false&language=tr-tr&page=${page}`;
        return this.http.get<GenericResponse<TvShow[]>>(url)
    }

    searchCasts(key: string, page: number): Observable<GenericResponse<Cast[]>> {
        const url = `${this.basePath}/search/person?query=${key}&include_adult=false&language=tr-tr&page=${page}`;
        return this.http.get<GenericResponse<Cast[]>>(url)
    }

    searchCompany(key: string, page: number): Observable<GenericResponse<Company[]>> {
        const url = `${this.basePath}/search/company?query=${key}&page=${page}`;
        return this.http.get<GenericResponse<Company[]>>(url)
    }
